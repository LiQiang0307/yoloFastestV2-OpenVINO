...
class yolo_fast_v2():
    def __init__(self, objThreshold=0.3, confThreshold=0.3, nmsThreshold=0.4):
        ...
        self.inpWidth = 352
        self.inpHeight = 352
        self.net = cv2.dnn.readNet('yolo-fastestv2-opencv.onnx')
    ...
    def detect(self, srcimg):
        blob = cv2.dnn.blobFromImage(
            srcimg, 1 / 255.0, (self.inpWidth, self.inpHeight))
        self.net.setInput(blob)
        outs = self.net.forward(self.net.getUnconnectedOutLayersNames())[0]
        print(outs.shape, "out shape")

        outputs = np.zeros(
            (outs.shape[0]*self.anchor_num, 5+len(self.classes)))
        row_ind = 0
        for i in range(len(self.stride)):
            h, w = int(self.inpHeight /
                       self.stride[i]), int(self.inpWidth / self.stride[i])
            length = int(h * w)
            grid = self._make_grid(w, h)
            for j in range(self.anchor_num):
                top = row_ind+j*length
                left = 4*j
                outputs[top:top + length, 0:2] = (
                    outs[row_ind:row_ind + length, left:left+2] * 2. - 0.5 + grid) * int(self.stride[i])
                outputs[top:top + length, 2:4] = (outs[row_ind:row_ind + length, left+2:left+4] * 2) ** 2 * np.repeat(
                    self.anchors[i, j, :].reshape(1, -1), h * w, axis=0)
                outputs[top:top + length, 4] = outs[row_ind:row_ind +
                                                    length, 4*self.anchor_num+j]
                outputs[top:top + length, 5:] = outs[row_ind:row_ind +
                                                     length, 5*self.anchor_num:]
            row_ind += length
        return outputs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgpath', type=str,
                        default='img/000004.jpg', help="image path")
    parser.add_argument('--objThreshold', default=0.1,
                        type=float, help='object confidence')
    parser.add_argument('--confThreshold', default=0.1,
    ...
    model = yolo_fast_v2(objThreshold=args.objThreshold,
