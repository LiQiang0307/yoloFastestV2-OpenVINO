...
version: 
Author: LiQiang
Date: 2021-09-18 10:05:35
LastEditTime: 2021-11-18 17:10:46
'''
import torch
...
class Detector(nn.Module):
    def __init__(self, classes, anchor_num, load_param, export_onnx=False):
        ...
        self.output_reg_layers = nn.Conv2d(
            out_depth, 4 * anchor_num, 1, 1, 0, bias=True)
        self.output_obj_layers = nn.Conv2d(
            out_depth, anchor_num, 1, 1, 0, bias=True)
        self.output_cls_layers = nn.Conv2d(
            out_depth, classes, 1, 1, 0, bias=True)
    ...
    def forward(self, x):
        ...
        if self.export_onnx:
            ...
            #     torch.cat((out_reg_3, out_obj_3, out_cls_3),
            #               1).permute(0, 2, 3, 1)
